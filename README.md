...
3. **Learn the interactions:** follow a max-three-choice-per-layer design tree, study six patterns, explore enlarged research animations, and open the eight-step executable Google Colab studio.
...
The versioned notebook lives at `notebooks/INFOSCI301_Interaction_Design_Companion.ipynb` and is reproducibly generated by `scripts/build-notebook.mjs`. It uses Plotly, pydeck, Altair and ipywidgets for the bilingual globe, spatial layer, linked selection, time, decision and evidence-card exercises. The app embeds a direct notebook download and links to its Colab runtime; no account or API key is required to read the embedded tutorial preview.
